...
const GB_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GB_API_KEY}`;
...
async function gbSend() {
  ...
  try {
    ...
    if (data.error) {
      ...
      if (code === 400) friendly = '⚠️ Invalid API key. Please check gogbot.js and make sure the key has no quotes or semicolons.';
      ...
      if (code === 404) friendly = '⚠️ Model not found. Please contact George to update the bot.';
