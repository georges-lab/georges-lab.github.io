...
/* ── Use gemini-1.5-flash — stable and free tier supported ── */
const GB_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${GB_API_KEY}`;
...
async function gbSend() {
  ...
  try {
    ...
    if (data.error) {
      ...
      let friendly = `⚠️ Gemini API error (${code}): ${msg}`;
      if (code === 400) friendly = '⚠️ Bad request — the API key may be wrong or the model name changed.';
      if (code === 403) friendly = '⚠️ API key does not have permission. Check aistudio.google.com.';
    ...
    gbRemoveTyping();
    gbAddMsg(
      `⚠️ Network error: ${err.message}.\n\nPlease check your internet connection or reach George directly:\n**WhatsApp:** +254710823964\n**Email:** ndungugeorge065@gmail.com`,
      'bot', 'hire'
    );
  }
